...
export async function GET(req) {
  try {
    ...
    if (session.user.role === 'admin') {
      // адмін бачить все
      const cc = await rq(`SELECT cc.*, 'client' AS type, cl.first_name || ' ' || cl.last_name AS sender_name,
      ...
      const cr = await rq(`SELECT cr.*, 'contractor' AS type, co.first_name || ' ' || co.last_name AS sender_name,
